"""
PART 2 - Normalize "Works Completed" from both packages. Long table,
...
def _normalize_one(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "completion_date" in out.columns:
        out["completion_date"] = normalize_date_column(out["completion_date"])
    ...
    if "image" in out.columns:
        out["image_present"] = out["image"].apply(lambda v: bool(clean_text_series(pd.Series([v])).iloc[0]))
    else:
        out["image_present"] = False
    out = add_term_columns(out, mp_name_col="mp_name", prefix="completion")
    return out


def build_normalized_completed() -> tuple[pd.DataFrame, dict, dict]:
    df1, df2, prof1, prof2 = load_both_packages("completed")
    ...
    keep = [
        ...
        "completion_date", "image", "image_present", "disbursed_amount",
        "completion_term", "completion_term_confidence", "completion_term_evidence",
